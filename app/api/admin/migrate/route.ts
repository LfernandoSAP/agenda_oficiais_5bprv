...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$executeRawUnsafe(
      `ALTER TYPE "Posto" ADD VALUE IF NOT EXISTS 'ASP_TEN_PM' BEFORE 'P1';`
    );

    const valores = await prisma.$queryRawUnsafe(
      `SELECT enumlabel FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'Posto' ORDER BY e.enumsortorder;`
    );
